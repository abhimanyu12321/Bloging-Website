...
const EditBlog = (editBlog) => {
  ...
  const [articleBody, setArticleBody] = useState(editBlog.editBlog.articleBody);
  const [category, setCategory] = useState(editBlog.editBlog.category);
console.log(editBlog)
  ...
  const handleSubmit = async (e)=>{
    ...
    const blog = {authorDetails, title, description, image, articleBody, category}
    console.log(blog)
    const response = await fetch(`https://anmol-solr.onrender.com/api/blogs/edit/${editBlog.editBlog._id}`,{
        method:'PATCH',
  ...
  return (
    <div>
      <div>
        <h1>
          Edit Your Blog  <Edit style={{fontSize:40}}/>
        </h1>
      </div>
      ...
      <div>
        ...
        <Dropdown
          ...
          menu={{ 
            ...
            onSelect : (e)=>{setCategory(e.selectedKeys[0])}
            }} 
          trigger={'click'}
          arrow= {'true'}
    ...
    <div className='dropdown'>
      <Space>
        <p>{category} <DownOutlined style={{fontSize:25}} /></p>
      </Space>
    </div>
  </Dropdown>
        </div>
